...
export class DqRulesComponent implements OnInit, AfterViewInit {
  ...
  displayedColumns: string[] = ['id', 'name', 'description', 'ruleType', 'targetTable', 'actions'];
    ...
    return (data: DqRule, filter: string): boolean => {
      ...
      const globalMatch = !searchTerms.global ||
        (data.id?.toString().toLowerCase().includes(searchTerms.global) ||
          ...
          data.targetTable?.toLowerCase().includes(searchTerms.global));
      ...
      const tableMatch = !searchTerms.targetTable || data.targetTable?.toLowerCase().includes(searchTerms.targetTable);

      var result = globalMatch && idMatch && nameMatch && descMatch && typeMatch && tableMatch;
      if (result === undefined) {
        result = true;
      }
      return result;
