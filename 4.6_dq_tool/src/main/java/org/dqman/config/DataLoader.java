...
import static org.dqman.config.ProjectDataCreator.createProjects;
import static org.dqman.config.RuleDataCreator.createRules;
import static org.dqman.config.FlowDataCreator.createFlows;
...
public class DataLoader {
    ...
    CommandLineRunner initDatabase(DqProjectRepository dqProjectRepository,
            DqIntegrationRepository dqIntegrationRepository,
            DqRuleRepository dqRuleRepository,
            DqFlowRepository dqFlowRepository) {
        return args -> {
            // Create test data
            createProjects(dqProjectRepository);
            integrationDataCreator.createIntegrations(dqIntegrationRepository);
            createRules(dqRuleRepository);
