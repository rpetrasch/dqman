# Welcome to Data Quality

This is the official homepage for our book project "Data Quality for Software Engineers", hosted on GitHub Pages.

- Open access book (PDF) download: Please visit the [Download Page](https://sites.google.com/view/dq4swe/home)
- Code for the examples in the book: Hosted on [GitHub](https://github.com/rpetrasch/dqman)
- Discussion and feedback: Can be found on the [dicussions section](https://github.com/rpetrasch/dqman/discussions)
- Call for contribution / participation: Write us an email [Richard Petrasch](mailto:richardroland2004@gmail.com), [Roland Petrasch](mailto:roland.petrasch@gmail.com)

# Current Version
- Please note that the current version is a draft (preprint) of the book and the code examples: 0.8 (as of Nov. 2025)
- We appreciate your feedback, ideas, discussion, and other contributions
- The next update is planned for Dec. 2025

# Addtional Resources
- Will be published here soon.

